...
![rw-book-cover](https://www.ontestautomation.com/images/logo_large.png)

## Metadata
- Author: [[Bas Dijkstra]]
- Full Title: Writing tests with Claude Code - part 1 - initial results
- Category: #articles
- Summary: Claude Code quickly generated 23 tests with good coverage, saving much time compared to writing them manually. The tests covered most code paths but missed some important boundary cases and critical logic. The author plans to improve the test suite using Claude and mutation testing in a future post.
- URL: https://www.ontestautomation.com/writing-tests-with-claude-code-part-1-initial-results/

## Full Document
This post was published on March 9, 2026

In [a recent post](https://www.ontestautomation.com/refactoring-the-rest-assured-net-code-with-claude-code/), I wrote about how I used Claude Code to analyze the code for RestAssured.Net and then perform a refactoring action, using hand-written tests as the safety net. In that post, I wrote that I didn’t want Claude to touch the tests themselves, and why. I was still curious, though, to find out for myself what Claude was capable of in terms of writing tests.

In this blog post, I’ll share with you some first steps in doing exactly that, and you’ll read about my thoughts and my thought process along the way. You’ll see how I create an initial suite of tests for a small Spring Boot-based API that I wrote for use in my [workshops](https://www.ontestautomation.com/training/), and how I think about and assess the results. In a follow-up blog post, I’ll show you how I improved the test suite based on my findings, again using Claude Code.

##### The starting point

As a starting point, I created a new repository containing the code for the API I use in my [mutation testing workshop](https://www.ontestautomation.com/training/mutation-testing/). I removed the existing tests, as we’re going to ask Claude to generate these for us. I also removed the README and the GitHub Actions build pipeline definition, as I want Claude to write tests based only on the product code itself, without being primed by other artifacts in the codebase. The only thing I left in are the dependencies used to write and run the tests, in this case REST Assured and JUnit.
...
What Claude did not do, probably because I didn’t explicitly ask for it, is add an abstraction layer to make the code easier to read, [such as the one described here](https://www.ontestautomation.com/using-the-client-test-model-in-rest-assured-net/). We’ll see how Claude does in this area in the next blog post, as I want to stick to assessing the quality of the initial output from Claude in this one.
...
![claude_mt_http500](https://www.ontestautomation.com/images/blog/claude_mt_http500.png)Mutation testing: surviving mutants for HTTP 500 path
...
![claude_mt_http204](https://www.ontestautomation.com/images/blog/claude_mt_http204.png)Mutation testing: surviving mutants for HTTP 204 path
...
![claude_mt_boundaries](https://www.ontestautomation.com/images/blog/claude_mt_boundaries.png)Mutation testing: surviving mutants for boundary values
...
Most importantly, I’m motivated / I feel like I am morally obliged to do so, because I deeply value writing tests that test meaningful things and that are actually able to detect changes in product behaviour. If all I cared about was having *some* tests to cover the API and declared, for example, 90% line coverage as ‘good enough’, I would be done by now.
...
There is some room for improvement in terms of readability of the tests, but that can probably be resolved by being more specific in my prompt and / or using dedicated [Claude Code skills](https://code.claude.com/docs/en/skills). I’ll explore and write about that soon.
