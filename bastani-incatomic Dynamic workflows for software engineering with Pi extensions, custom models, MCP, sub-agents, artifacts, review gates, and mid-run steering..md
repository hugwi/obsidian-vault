...
categories:
  - "[[Resources]]"
domain: engineering
title: "bastani-inc/atomic: Dynamic workflows for software engineering with Pi extensions, custom models, MCP, sub-agents, artifacts, review gates, and mid-run steering."
source: "https://github.com/bastani-inc/atomic"
author:
published:
created: 2026-06-07
description: "Dynamic workflows for software engineering with Pi extensions, custom models, MCP, sub-agents, artifacts, review gates, and mid-run steering. - bastani-inc/atomic"
tags:
  - "to-process"
  - orchestration
---
...
[![atomic-promo](https://github.com/bastani-inc/atomic/raw/main/assets/atomic-promo.gif)](https://github.com/bastani-inc/atomic/blob/main/assets/atomic-promo.gif)
...
*An open-source, model-agnostic take on dynamic workflows for software engineering — with Pi extensions, custom models, MCP, sub-agents, artifacts, and review gates.*

[**Get started →**](#get-started) · [Spec-driven development](#spec-driven-development) · [When to use Atomic](#when-should-i-use-atomic) · [FAQ](#faq) · [Docs](https://docs.bastani.ai/)
...
See [Providers & Models](https://github.com/bastani-inc/atomic/blob/main/packages/coding-agent/README.md#providers--models) for the full provider list (API keys + subscriptions). For non-interactive use, `atomic -p "<prompt>"` prints the response and exits.

> ⚠️
> 
> Workflows run with agent permission checks **disabled** so pipelines don't block on prompts. Run autonomous workflows inside a devcontainer, VM, or remote dev machine — not your host machine.

> ⚠️
> 
> **Temporary workflow migration note:** Recent workflow authoring changes may require updates to custom workflows, especially workflows that import other workflows by registered name or path object. Workflow imports now use normal TypeScript module imports and pass compiled workflow definitions to `.import(workflow, { as? })`. If an existing workflow no longer loads, ask Atomic or your coding agent to update it using the [workflow import guidance](https://github.com/bastani-inc/atomic/blob/main/packages/workflows/README.md#example-4--compose-workflows-with-imports).

**Prerequisites, devcontainer**

**Prerequisites** — install Node.js 24 LTS+, a global package manager, model-provider access, and a compatible terminal. See [Providers & Models](https://github.com/bastani-inc/atomic/blob/main/packages/coding-agent/README.md#providers--models) and [Terminal setup](https://github.com/bastani-inc/atomic/blob/main/packages/coding-agent/docs/terminal-setup.md).
...
See [Programmatic Usage](https://github.com/bastani-inc/atomic/blob/main/packages/coding-agent/README.md#programmatic-usage) for the SDK and RPC entry points.
...
| *author your own* | Anything outside the built-ins: issue-to-PR, review-to-merge, migration, triage, release, compliance, or team-specific review pipelines. Describe the process in natural language and Atomic can scaffold a `defineWorkflow()` file with typed CLI flags. | *"Create a reusable workflow that takes an issue, writes a plan, creates a branch, runs implementation and review stages, runs tests and lint, then stops for approval before final output."* |

Run `/workflow list` to see installed workflows and `/workflow inputs <name>` for input schemas. `/workflow status <id>`, `/workflow connect <id>`, and `/workflow resume <id>` manage running or paused runs. Runnable references live in [`packages/coding-agent/examples/`](https://github.com/bastani-inc/atomic/blob/main/packages/coding-agent/examples).
...
Sub-agents are purpose-built agents with scoped context, tools, and termination conditions. Atomic bundles **8 sub-agents** from [`packages/subagents/agents/`](https://github.com/bastani-inc/atomic/blob/main/packages/subagents/agents). Workflows and skills use them to split large jobs into smaller, auditable passes.
...
The docs are open source in this repository under [`packages/coding-agent/docs`](https://github.com/bastani-inc/atomic/blob/main/packages/coding-agent/docs). Open a PR against this project to suggest a change.
...
See [CONTRIBUTING.md](https://github.com/bastani-inc/atomic/blob/main/CONTRIBUTING.md) for contribution guidelines and [DEV\_SETUP.md](https://github.com/bastani-inc/atomic/blob/main/DEV_SETUP.md) for development setup and testing details.

Looking to contribute workflows? Check out the atomic-workflows repo [here](https://github.com/lavaman131/atomic-workflows).

## License

MIT — see [LICENSE](https://github.com/bastani-inc/atomic/blob/main/LICENSE).
