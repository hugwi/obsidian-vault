...
![rw-book-cover](https://martinfowler.com/articles/harness-engineering/card.png)

## Metadata
- Author: [[Birgitta Böckeler]]
- Full Title: Harness engineering for coding agent users
- Category: #articles
- Summary: Coding agents work best when paired with a "harness," which guides their actions and helps them self-correct errors. This harness uses both feedforward controls to steer the agent before acting and feedback sensors to catch mistakes after. Building strong harnesses takes effort but boosts trust and accuracy in AI-driven coding.
...
![Three concentric circles, with the model in the core (the ultimate thing being harnessed), then the coding agent's builder harness next circle out, and the coding agent's user harness as the outermost ring](https://martinfowler.com/articles/harness-engineering/harness-bounded-contexts.png)Figure 1: The term “harness” means different things depending on the bounded context.
