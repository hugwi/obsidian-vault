...
| `"[[Raw]]"` | external source notes (lives in `Raw/`) |
...
### Adding new notes
Place at root (or `References/` for a person, `Raw/` for external source notes). Frontmatter:
```yaml
---
created: YYYY-MM-DD
categories:
...
- File names: human-readable; filename is the note title. **Android-safe filenames are mandatory**: use only letters, numbers, spaces, hyphens, underscores, parentheses, and periods. Never create or rename files/folders with `" * / : < > ? \\ |`; also avoid emoji and unusual Unicode punctuation in filenames. This is especially important for clipping titles because Android may reject them during checkout.
- When creating a note from an external title, sanitize the filename before writing it; preserve the original title inside the note/frontmatter instead. If a generated filename is unsafe, replace forbidden characters with `-`, collapse repeated separators, trim whitespace, and keep the extension.
