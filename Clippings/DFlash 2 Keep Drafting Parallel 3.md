...
An `AGENTS.md` file is a markdown file you check into Git that customizes how AI coding agents behave in your repository. It sits at the top of the conversation history, right below the [system prompt](https://www.aihero.dev/ai-coding-dictionary/system-prompt).
...
Many tools support "agent skills" - commands or workflows the agent can invoke to learn how to do something specific. These are another form of [progressive disclosure](https://www.aihero.dev/ai-coding-dictionary/progressive-disclosure): the agent pulls in knowledge only when needed.
